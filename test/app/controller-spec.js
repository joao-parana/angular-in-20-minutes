describe('testing directive password', function() {
	...
	it("should have the same name as user name in "
			+ "lowercase when the lastname is null", function() {
		scope.user.name = name;
		scope.$apply();
		expect(scope.user.username).toBe(name.toLowerCase());
	});

	it("should have the same name as user name in lowercase "
			+ "when the lastname is null", function() {
		scope.user.name = name;
